...
export const register = async (req, res) => {
  ...
  if (typeof email !== "string" || !email.includes("@")) return res.status(400).send("Valid email is required");
...
export const login = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ _id: user._id }, process.env.JWT_SECRET, {
      expiresIn: "7d",// expiry is given for seven days , can be changed
    });
...
export const userFollowing = async (req, res) => {
  try {
    const user = await User.findById(req.auth._id);
    const following = await User.find({ _id: user.following }).select("-password -secret").limit(50);
    res.json(following);
  } catch (err) {
    return res.status(400).send("Unable to complete the request.");
  }
...
export const sendFriendRequest = async (req, res) => res.status(501).send("This feature is not implemented yet.");

export const acceptFriendRequest = async (req, res) => res.status(501).send("This feature is not implemented yet.");

export const removeFriend = async (req, res) => res.status(501).send("This feature is not implemented yet.");

export const getFriendsPosts = async (req, res) => res.status(501).send("This feature is not implemented yet.");